...
public class CookieUtil {
    private static int accessTokenMaxAge = 60 * 30;
    private static int refreshTokenMaxAge = 60 * 60 * 24 * 30 * 1;
    ...
    public static ResponseCookie createDeleteTokenCookie(String name, Boolean isLocal) {
        ResponseCookie.ResponseCookieBuilder deleteTokenCookie = ResponseCookie.from(name, null)
                .secure(true)
                .httpOnly(true)
                .domain(isLocal ? "localhost" : staticSecurityProperties.getDefaultHost())
                .path("/")
                .sameSite("None")
                .maxAge(0);
        return deleteTokenCookie.build();
    }
}
